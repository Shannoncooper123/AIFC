...
# 导入指标计算函数
from monitor_module.indicators.volatility import (
    calculate_ema_list,
    calculate_rsi_list,
...
def _plot_candlestick_chart(klines: List[Kline], symbol: str, interval: str, visible_count: int) -> str:
    ...
    # 绘制Bollinger Bands
    valid_indices = [i for i in indices if plot_bb_upper[i] is not None]
    if valid_indices:
        ...
        ax_main.plot(valid_indices, [plot_bb_middle[i] for i in valid_indices], 
                    color='#9c27b0', alpha=0.6, linewidth=1.5, linestyle='--', label='BB Mid', zorder=4)
        ax_main.plot(valid_indices, [plot_bb_lower[i] for i in valid_indices], 
                    color='#9c27b0', alpha=0.4, linewidth=1, label='BB Lower', zorder=4)
        ax_main.fill_between(valid_indices, 
                            [plot_bb_upper[i] for i in valid_indices],
                            [plot_bb_lower[i] for i in valid_indices],
                            color='#9c27b0', alpha=0.05, zorder=1)
    
    # 绘制EMA
    valid_indices = [i for i in indices if plot_ema_fast[i] is not None]
    if valid_indices:
        ax_main.plot(valid_indices, [plot_ema_fast[i] for i in valid_indices], 
                    color='#ff9800', linewidth=1.5, label='EMA7', zorder=5)
    
    valid_indices = [i for i in indices if plot_ema_slow[i] is not None]
    if valid_indices:
    ...
    valid_indices = [i for i in indices if plot_hist[i] is not None]
    if valid_indices:
    ...
    valid_indices = [i for i in indices if plot_macd[i] is not None]
    if valid_indices:
        ax_macd.plot(valid_indices, [plot_macd[i] for i in valid_indices], 
                    color='#2196f3', linewidth=1.5, label='MACD')
        ax_macd.plot(valid_indices, [plot_signal[i] for i in valid_indices], 
    ...
    valid_indices = [i for i in indices if plot_rsi[i] is not None]
    if valid_indices:
...
def get_kline_image_tool(
    ...
    Args:
        ...
        feedback: 当前分析进度总结，详细说明当前的分析阶段以及洞察，并给出下一步的计划
    ...
    try:
        ...
        # 记录工具调用
        logger.info(f"get_kline_image_tool 被调用 - symbol={symbol}, interval={interval}, limit={limit}, feedback={feedback}")
