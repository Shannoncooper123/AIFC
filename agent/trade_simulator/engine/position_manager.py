...
class PositionManager:
    ...
    def open_position(self, symbol: str, side: str, quote_notional_usdt: float, leverage: int,
        ...
        with self.lock:
            ...
            # 获取入场价格
            entry = self.get_latest_close_price(symbol)
            if entry is None:
    ...
    def close_position(self, position_id: Optional[str] = None, symbol: Optional[str] = None,
        ...
        with self.lock:
            ...
            original_notional = pos.notional_usdt  # 保存原始值用于历史记录
            ...
            fee_close = notional_close * self.cfg.taker_fee_rate
            # 更新余额：加上盈亏，扣除手续费
            self.account.balance += pnl - fee_close
