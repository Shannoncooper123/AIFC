...
MAIN="$BASE_DIR/agent/agent_main.py"
...
# 启动前先清理旧进程
echo "检查并清理旧的 agent 进程..."
PIDS=$(ps aux | grep "python.*agent_main.py" | grep -v grep | awk '{print $2}' || true)
...
if [[ -n "$PIDS" ]]; then
  ...
  for i in {1..30}; do
    sleep 1
    REMAINING=$(ps aux | grep "python.*agent_main.py" | grep -v grep | wc -l | tr -d ' ')
