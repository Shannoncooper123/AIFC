...
def update_tp_sl_tool(symbol: str,
    ...
    在持仓维持期间，动态调整TP/SL以适应波动与风险控制。止盈止损必须使用绝对价格，不支持百分比。
    建议：浮盈≥1R 将 SL 移至入场价附近（BE）；浮盈≥2R 启用追踪止盈。
    
    成功时返回更新后的持仓结构化字典，失败时返回 {"error": "..."}。
    ...
    try:
        ...
        if error:
            logger.error(f"update_tp_sl_tool: {error}")
        ...
        if pos:
            ...
            if current_tp == tp_price and current_sl == sl_price:
                ...
                return make_input_error(f"当前仓位的止盈止损价格与您要设置的值完全相同（TP={tp_price}, SL={sl_price}），无需更新。如果需要调整风控策略，请设置不同的价格。")
            ...
            if side == 'long':
                if sl_price > entry_price * max_reasonable_multiplier:
                    ...
                    return make_input_error(f"做多仓位的止损价格 {sl_price} 异常：远高于入场价 {entry_price:.6f}（超过{max_reasonable_multiplier}倍）。这可能是小数点错误，例如将 0.{str(sl_price).replace('.', '')} 误写成了 {sl_price}。请检查并修正。")
                if tp_price < entry_price * min_reasonable_multiplier:
                    ...
                    return make_input_error(f"做多仓位的止盈价格 {tp_price} 异常：远低于入场价 {entry_price:.6f}（低于1/{max_reasonable_multiplier}）。这可能是小数点错误。请检查并修正。")
            elif side == 'short':
                if sl_price < entry_price * min_reasonable_multiplier:
                    ...
                    return make_input_error(f"做空仓位的止损价格 {sl_price} 异常：远低于入场价 {entry_price:.6f}（低于1/{max_reasonable_multiplier}）。这可能是小数点错误。请检查并修正。")
                if tp_price > entry_price * max_reasonable_multiplier:
                    ...
                    return make_input_error(f"做空仓位的止盈价格 {tp_price} 异常：远高于入场价 {entry_price:.6f}（超过{max_reasonable_multiplier}倍）。这可能是小数点错误。请检查并修正。")
            
            if side == 'long':
                if tp_price <= sl_price:
                    logger.error(f"update_tp_sl_tool: LONG仓位TP/SL关系错误 tp_price={tp_price} <= sl_price={sl_price}")
                    return make_input_error(f"做多仓位的止盈价格必须高于止损价格。当前 tp_price={tp_price} <= sl_price={sl_price}。请检查是否把止盈止损价格传反了。")
            elif side == 'short':
                if sl_price <= tp_price:
                    logger.error(f"update_tp_sl_tool: SHORT仓位TP/SL关系错误 sl_price={sl_price} <= tp_price={tp_price}")
                    return make_input_error(f"做空仓位的止损价格必须高于止盈价格。当前 sl_price={sl_price} <= tp_price={tp_price}。请检查是否把止盈止损价格传反了。")
