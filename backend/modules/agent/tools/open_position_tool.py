...
def open_position_tool(
    ...
    当 side="NULL" 时：用于标记"不执行任何开仓"，系统将自动切换到持仓管理阶段，此时其他参数可省略。
    当 side="BUY"/"SELL" 时：执行实际开仓或加仓操作，在全仓保证金模式下以当前价开仓或加仓。
    
    成功时返回格式化的开仓信息字符串（含入场价、数量、止盈止损、风险回报比），失败时返回 {"error": "..."}。
    注意：开仓金额不建议超过可用保证金的5%；建议先调用 get_kline_image 分析多周期K线图像。
