...
支持手动设置图片的质量，取值范围`high`、`low`。具体使用教程见[理解图像的深度控制](https://www.volcengine.com/docs/82379/1362931#bf4d9224)。
...
取值范围：`high`、`low`。
支持手动设置图片的质量。具体使用教程见[理解图像的深度控制](https://www.volcengine.com/docs/82379/1362931#bf4d9224)。
...
控制模型是否开启深度思考模式。
> 不同模型的默认取值不同，支持此字段的模型以及使用示例请参见[文档](https://www.volcengine.com/docs/82379/1449737#0002)。
...
是否使用[TPM保障包](https://www.volcengine.com/docs/82379/1510762)。取值范围：`auto`、`default`。
...
> 仅模型doubao\-seed\-1\-6\-lite\-251015、doubao\-seed\-1\-6\-251015支持该字段，使用说明见[文档](https://www.volcengine.com/docs/82379/1449737#%E8%B0%83%E8%8A%82%E6%80%9D%E8%80%83%E9%95%BF%E5%BA%A6)。
...
* **thinking.type ** 取值为`disabled`：**reasoning_effort ** 仅支持取值`minimal`；配置为`low`、`medium`、`high`时，请求报错。
